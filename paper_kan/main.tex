...
\begin{abstract}
This paper presents an extended empirical study of Kolmogorov-Arnold Networks (KAN) and hybrid KAN architectures in forecasting tasks with markedly different structure, data-generating mechanisms, and practical constraints. The first case study addresses multivariate environmental time series forecasting on the UCI Air Quality dataset, where KAN-based models are compared with recurrent and feed-forward neural baselines in standard and robustness-oriented regimes. The second case study focuses on short-term panel forecasting on a leakage-safe dataset constructed from Russian regional market data, where the learning problem is complicated by mixed-frequency inputs, regional heterogeneity, stochastic fluctuations, and strict causal alignment requirements. In the baseline panel setup, strong tabular models outperform direct KAN formulations by a substantial margin. However, after reformulating the target as a next-step first-order difference, incorporating region embeddings, and introducing residual learning, the strongest KAN-based model approaches the performance of the best boosting baseline much more closely. Across both case studies, the experiments show that the main value of KAN is not universal predictive dominance, but the combination of nonlinear modeling flexibility, architectural modularity, and analysis of learned channel-wise $\phi$-functions. The paper therefore contributes not only a benchmark-style comparison, but also a problem-design perspective: KAN is most useful when the forecasting task is formulated in a way that matches its inductive bias and when nonlinear feature-channel analysis is treated as a first-class modeling objective.
\end{abstract}

\begin{IEEEkeywords}
Kolmogorov-Arnold Networks, forecasting, time series, panel data, first-order differences, stochastic dynamics, hybrid neural models
\end{IEEEkeywords}
...
\end{equation}
Equivalently, a forecasting window can be written as a matrix
\begin{equation}
\mathbf{X}_{t,w} =
\begin{bmatrix}
x_{t-w+1}^{(1)} & x_{t-w+2}^{(1)} & \cdots & x_t^{(1)} \\
x_{t-w+1}^{(2)} & x_{t-w+2}^{(2)} & \cdots & x_t^{(2)} \\
\vdots & \vdots & \ddots & \vdots \\
x_{t-w+1}^{(d)} & x_{t-w+2}^{(d)} & \cdots & x_t^{(d)}
\end{bmatrix}
\in \mathbb{R}^{d \times w}.
\end{equation}
This matrix view is important because it makes explicit that the model receives both temporal structure and cross-channel dependencies. In the climate case, the rows correspond to pollutant and meteorological variables. In the panel case, they correspond to price history, load-related variables, calendar encodings, lagged losses, and region-dependent signals.
...
For the strengthened panel experiments we use first-order differences:
\begin{equation}
\Delta y_{r,t} = y_{r,t+1} - y_{r,t}.
\end{equation}
The model predicts $\Delta y_{r,t}$ and the final level forecast is reconstructed as
\begin{equation}
\hat{y}_{r,t+1} = y_{r,t} + \widehat{\Delta y}_{r,t}.
\end{equation}
This formulation is better aligned with short-term stochastic dynamics, where recent shocks, local increments, and mean-reverting behavior can be easier to learn than absolute levels.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\textwidth]{multivariate_series_matrix.png}
\caption{Example of a multivariate forecasting window represented as a standardized feature-time matrix. This view makes the transition from a single temporal series to a multichannel forecasting object explicit.}
\label{fig:multivariate_matrix}
\end{figure}
...
In iterative multi-step forecasting, the one-step model can be reused recursively:
\begin{equation}
\hat{x}_{t+k} = F_{\theta}(\hat{x}_{t+k-w}, \ldots, \hat{x}_{t+k-1}), \quad k=1,\ldots,h,
\end{equation}
where observed values are used for the initial window and predicted values are fed back for later steps. This iterative scheme is relevant for seasonal and environmental data because errors may accumulate differently across daily, weekly, and annual periodic components.

For seasonal channels, a compact Fourier representation can be written as
\begin{equation}
s(t) = a_0 + \sum_{m=1}^{M} \left[
a_m \sin\left(\frac{2\pi m t}{P}\right) +
b_m \cos\left(\frac{2\pi m t}{P}\right)
\right],
\end{equation}
where $P$ is a period such as a day, week, or year. This representation motivates the use of cyclical features such as sine and cosine encodings in the panel dataset and helps separate deterministic periodicity from more stochastic residual dynamics.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\textwidth]{fourier_seasonality_components.png}
\caption{Illustration of seasonal structure as a sum of sine and cosine components. Such encodings are useful when data contain daily, weekly, or annual periodicity.}
\label{fig:fourier}
\end{figure}
...
\subsection{Formula-Level Description of Learned Channel Functions}

The nonlinear transformations learned by KAN are not only visual artifacts. Each edge function can be written explicitly as a basis expansion:
\begin{equation}
\phi_{j,q}(z_j) = \sum_{m=1}^{M} c_{j,q,m} B_m(z_j),
\end{equation}
where $z_j$ is a standardized input feature, $B_m(\cdot)$ is a one-dimensional basis function, and $c_{j,q,m}$ are learned coefficients for the transformation from input channel $j$ to hidden unit $q$. Therefore, the contribution of a feature group $G$ can be summarized as
\begin{equation}
\Phi_G(x_G) = \sum_{j \in G} \sum_{q} \phi_{j,q}(z_j).
\end{equation}

For presentation and reporting, the sampled learned $\phi$-functions were also approximated by low-order polynomials:
\begin{equation}
\hat{\phi}_{j,q}(z) = a_3 z^3 + a_2 z^2 + a_1 z + a_0.
\end{equation}
These polynomial expressions should be interpreted as compact summaries of the learned spline-like functions, not as constraints imposed during training. Table~\ref{tab:phi_formula_summary} gives representative examples for the most important channels.

\begin{table}[t]
\centering
\caption{Polynomial summaries of representative learned KAN channel functions}
\label{tab:phi_formula_summary}
\small
\begin{tabular}{llc}
\toprule
Channel & Approximate learned function $\hat{\phi}(z)$ & $R^2$ \\
\midrule
Current price &
$-0.0106z^3 + 0.0179z^2 + 0.1462z - 0.0925$ & 0.969 \\
Price lag 1 &
$0.0060z^3 - 0.0042z^2 - 0.1149z + 0.0869$ & 0.995 \\
Hour cosine &
$0.0041z^3 - 0.0366z^2 - 0.0824z + 0.1197$ & 0.875 \\
Hybrid current price &
$0.0119z^3 - 0.0026z^2 - 0.1832z + 0.0477$ & 0.999 \\
Hybrid sine hour &
$0.0040z^3 - 0.0392z^2 - 0.0534z + 0.0919$ & 0.859 \\
\bottomrule
\end{tabular}
\end{table}

At the feature-group level, the resulting nonlinear correction can therefore be described schematically as
\begin{equation}
f_{\text{KAN}}(x) =
\Phi_{\text{price}}(x_{\text{price lags}}) +
\Phi_{\text{calendar}}(x_{\sin h},x_{\cos h}) +
\Phi_{\text{load}}(x_{\text{load lags}}) +
\Phi_{\text{region}}(e_r) +
\epsilon_{\theta}.
\end{equation}
This expression is useful for thesis presentation because it connects the graphical $\phi$-function interpretation with an explicit formula-level description of the channels.
...
The dataset was used in four experimental variants:
\begin{itemize}
    \item \textbf{Full}: the cleaned multivariate series with standard chronological splitting;
    \item \textbf{Missing}: the same series with additional missing observations to test robustness;
    \item \textbf{Noise}: additive perturbations to emulate sensor uncertainty and stochastic measurement error;
    \item \textbf{Small}: reduced training size to test behavior under limited data.
\end{itemize}

These variants were selected because environmental data often contain seasonal and annual regularities, but also random measurement fluctuations. Therefore, the experiments deliberately combine periodic structure with stochastic degradation scenarios.
...
\subsection{Analysis of Learned Nonlinear Effects}
...
\textbf{3) Residual learning.} We use a baseline model to explain the main component and then train KAN to model the remaining residual signal. In the first strengthened experiment, Ridge was used as the base model. In the advanced residual experiment, the base component was replaced by a stronger HistGradientBoosting model and the KAN-style spline branch was trained as a nonlinear calibration layer on the validation segment. This is particularly appealing because it allows KAN to focus on nonlinear residual effects rather than on the full target from scratch.
...
smallbox/.style={draw, rounded corners, align=center, minimum width=2.8cm, minimum height=0.82cm},
arr/.style={-{Latex[length=2.5mm]}, thick}
]
\node[box, fill=blue!8] (x) {Input $x_t$\\ price lags, calendar, load, region, losses};
\node[box, fill=orange!12, below=of x] (base) {Base model\\ HistGradientBoosting};
\node[smallbox, fill=orange!8, below=of base] (basepred) {Base forecast\\ $\hat{y}_{base}$};
\node[smallbox, fill=gray!10, below left=of basepred] (resid) {Residual on validation\\ $r = y - \hat{y}_{base}$};
\node[box, fill=yellow!12, below right=of basepred] (kan) {KAN-style residual branch\\ spline / $\phi$-functions};
\node[smallbox, fill=yellow!8, below=of kan] (kanpred) {Residual correction\\ $\hat{y}_{res}$};
\node[box, fill=red!8, below=2.1cm of basepred] (out) {Final forecast\\ $\hat{y} = \hat{y}_{base} + \hat{y}_{res}$};

\draw[arr] (x) -- (base);
\draw[arr] (base) -- (basepred);
\draw[arr] (basepred) -- (resid);
\draw[arr] (resid) -- (kan);
\draw[arr] (x.east) -| (kan.north);
\draw[arr] (kan) -- (kanpred);
\draw[arr] (basepred) -- (out);
\draw[arr] (kanpred) -- (out);
\end{tikzpicture}
}
\caption{Advanced residual hybrid KAN architecture. A strong boosting model captures the main one-step dynamics, while the KAN-style branch learns a nonlinear residual correction through interpretable spline-based channel functions.}
...
Model & RMSE & MAE & $R^2$ \\
\midrule
AdvancedResidualHybridKAN & 99.96 & 55.95 & 0.945 \\
HGBDeltaStrong & 101.41 & 53.51 & 0.944 \\
HybridKANEmbedDelta & 107.57 & 61.15 & 0.937 \\
ResidualRidgeKAN & 108.56 & 60.81 & 0.936 \\
KANApproxDelta & 109.33 & 65.09 & 0.935 \\
RidgeDelta & 115.74 & 71.03 & 0.927 \\
\bottomrule
\end{tabular}
\end{table}

The strongest KAN-related model became AdvancedResidualHybridKAN with RMSE 99.96. This model combines a strong delta-target boosting baseline with a KAN-style residual calibration branch. It improves over the previous best boosting-only result and represents a substantial improvement compared to the original Hybrid KAN result of RMSE 181.23.
...
\midrule
KAN $\rightarrow$ KANApproxDelta & 149.51 & 109.33 \\
Hybrid KAN $\rightarrow$ AdvancedResidualHybridKAN & 181.23 & 99.96 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\textwidth]{electricity_channel_correlations.png}
\caption{Mutual correlations between major panel forecasting channels. The panel task contains strong persistence, cross-channel dependence, and stochastic residual variation.}
\label{fig:channel_corr}
\end{figure}
